...
export default function WalletPage({ onBack }: { onBack?: () => void } = {}) {
  ...
  const loadPackages = async () => {
    ...
    const { data: settings } = await supabase
      .from("app_settings")
      .select("*")
      .in("key", ["primary_gateway", "manual_deposit_link", "extension_whatsapp_link", "extension_button_enabled", "extension_button_text"]);
    ...
    if (settings) {
      ...
      const ewl = settings.find(s => s.key === "extension_whatsapp_link");
      if (ewl) setExtensionWhatsappLink(ewl.value as any);

      const ebe = settings.find(s => s.key === "extension_button_enabled");
      if (ebe) setExtensionButtonEnabled(ebe.value === "true");

      const ebt = settings.find(s => s.key === "extension_button_text");
      if (ebt) setExtensionButtonText(ebt.value as any);
    }
  };
